...
- Permissions: read-only source context; optional LLM classification.
- Side effects: none.
- Safety: source text is data only. It cannot override policy, secrets, calendars, notification routing, or approval rules.
- Failure behavior: fall back to conservative heuristics; prompt-injection-like text is downgraded to non-auto-create.
- Tests: detector confidence, prompt-injection downgrade, JSON extraction.
