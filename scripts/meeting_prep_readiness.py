#!/usr/bin/env python3
"""Read-only readiness gate for Rocky meeting prep briefing lane."""
...
from assistant_scheduler_health import MEETING_PREP_BRIEFING_SPEC
...
def evaluate_meeting_prep_readiness(
    *,
    ...
    expected_run = _expected_run(target, now=now)
    grace_until = expected_run + timedelta(minutes=30)
    if now < grace_until:
        status = READY_PENDING
        reason = "meeting_prep_first_natural_run_pending"
    else:
        status = READY_VERIFIED
        reason = "meeting_prep_natural_run_verified"

    launch = launchagent_payload or inspect_launchagent(spec.launchagent, now=now).to_dict()
    state_path = Path(state_file or spec.state_path or "")
    state_payload = _read_json(state_path) if state_path else {}
    ...
    recent_for_date = _recent_for_date(runs, target)
    ...
    if now >= grace_until and not recent_for_date:
        issues.append({"source": "run_history", "reason": "meeting_prep_run_missing_after_grace"})
    if now >= grace_until and state_payload and str(state_payload.get("target_date") or "") != target.isoformat():
        issues.append({"source": "state", "reason": "meeting_prep_state_stale", "state_target_date": state_payload.get("target_date")})
    if issues:
        status = NOT_READY if any(item["source"] in {"launchagent", "stderr", "run_history"} for item in issues) else MANUAL_REVIEW
        reason = "meeting_prep_readiness_evidence_failed"
    payload = {
        "status": status,
        "reason": reason,
        ...
        "evidence": {
            "launchagent": _safe_launch(launch),
            "stderr": stderr_status,
            "state": _safe_state(state_payload),
            "recent_run": _safe_run(recent_for_date),
...
def _expected_run(day: date, *, now: datetime) -> datetime:
    candidate = datetime.combine(day, datetime.min.time().replace(hour=7, minute=30), tzinfo=ZoneInfo(TIMEZONE))
    if day.weekday() >= 5:
        offset = 7 - day.weekday()
        candidate = datetime.combine(day + timedelta(days=offset), datetime.min.time().replace(hour=7, minute=30), tzinfo=ZoneInfo(TIMEZONE))
    return candidate


def _recent_for_date(runs: list[dict[str, Any]], target: date) -> dict[str, Any] | None:
    for run in runs:
...
def _stderr_status(path: Path) -> dict[str, Any]:
    ...
    if text:
        return {"status": "blocked", "reason": "stderr_not_empty", "chars": len(text)}
    return {"status": "ok", "chars": 0}
...
def _safe_state(payload: dict[str, Any]) -> dict[str, Any]:
    return {key: payload.get(key) for key in ("last_run_at", "last_status", "target_date", "reason", "processed_count", "sent_count", "calendar_write_attempted", "notion_write_attempted") if key in payload}
...
def _safe_run(run: dict[str, Any] | None) -> dict[str, Any] | None:
    if not run:
        return None
    summary = _parse_json(run.get("summary"))
    return {"run_id": run.get("run_id"), "status": run.get("status"), "idempotency_key": run.get("idempotency_key"), "created_at": run.get("created_at"), "summary": summary}


def _summary(status: str, *, issues: list[dict[str, Any]], target: date) -> str:
    if status == READY_VERIFIED:
        return f"Meeting prep is production-proven for {target.isoformat()}."
    if status == READY_PENDING:
        return f"Meeting prep is healthy but waiting for the first natural weekday run for {target.isoformat()}."
    return "Meeting prep is not production-ready: " + ", ".join(str(item.get("reason")) for item in issues)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate meeting prep readiness.")
    ...
    payload = evaluate_meeting_prep_readiness(expected_date=args.expected_date, now_local=args.now_local, scheduler_db_path=args.scheduler_db, state_file=args.state_file, stderr_path=args.stderr_path)
    if args.json_output:
