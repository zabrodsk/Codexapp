...
ASSISTANT_LEARNING_SSH_BRIDGE_PROGRAM_ARGUMENTS = [
    ...
    "StrictHostKeyChecking=accept-new",
    "localhost",
    "cd /Users/clawdbot/.openclaw/workspace && /Users/clawdbot/.openclaw/workspace/.venv/bin/python scripts/assistant_learning_scheduler.py --live --notify-failures --json",
]
ASSISTANT_LEARNING_DIRECT_PROGRAM_ARGUMENTS = [
    "/Users/clawdbot/.openclaw/workspace/.venv/bin/python",
    "/Users/clawdbot/.openclaw/workspace/scripts/assistant_learning_scheduler.py",
    "--live",
    "--notify-failures",
    "--json",
]
WEEKLY_PERSONAL_REVIEW_SSH_BRIDGE_PROGRAM_ARGUMENTS = [
    ...
    "StrictHostKeyChecking=accept-new",
    "localhost",
    "cd /Users/clawdbot/.openclaw/workspace && /Users/clawdbot/.openclaw/workspace/.venv/bin/python scripts/weekly_personal_review_scheduler.py --live --notify --json",
]
WEEKLY_PERSONAL_REVIEW_DIRECT_PROGRAM_ARGUMENTS = [
    "/Users/clawdbot/.openclaw/workspace/.venv/bin/python",
    "/Users/clawdbot/.openclaw/workspace/scripts/weekly_personal_review_scheduler.py",
    "--live",
    "--notify",
    "--json",
]
...
MEETING_OUTCOME_CAPTURE_SSH_BRIDGE_PROGRAM_ARGUMENTS = [
    ...
    "StrictHostKeyChecking=accept-new",
    "localhost",
    "cd /Users/clawdbot/.openclaw/workspace && /Users/clawdbot/.openclaw/workspace/.venv/bin/python scripts/meeting_outcome_scheduler.py --live --notify-failures --apply-safe-followups --json",
]
MEETING_OUTCOME_CAPTURE_DIRECT_PROGRAM_ARGUMENTS = [
    "/Users/clawdbot/.openclaw/workspace/.venv/bin/python",
    "/Users/clawdbot/.openclaw/workspace/scripts/meeting_outcome_scheduler.py",
    "--live",
    "--notify-failures",
...
ASSISTANT_LEARNING_SPEC = SchedulerJobSpec(
    ...
    launchagent=LaunchAgentSpec(
        ...
        plist_path="/Users/clawdbot/Library/LaunchAgents/com.openclaw.rocky-assistant-learning.plist",
        program_arguments=ASSISTANT_LEARNING_SSH_BRIDGE_PROGRAM_ARGUMENTS,
        working_directory="/Users/clawdbot/.openclaw/workspace",
        stdout_path="/Users/clawdbot/.openclaw/logs/rocky-assistant-learning.log",
        stderr_path="/Users/clawdbot/.openclaw/logs/rocky-assistant-learning.err.log",
        weekdays=[1, 2, 3, 4, 5],
        hour=20,
        minute=45,
        timezone="Europe/Prague",
        first_expected_run_after="2026-05-25T20:45:00+02:00",
    ),
    state_path="/Users/clawdbot/.openclaw/state/assistant_learning_scheduler.json",
    first_expected_run_after="2026-05-25T20:45:00+02:00",
)

WEEKLY_PERSONAL_REVIEW_SPEC = SchedulerJobSpec(
    job_name="weekly_personal_review",
    job_label="Rocky weekly personal review",
    workflow="weekly_personal_review_scheduler",
    launchagent=LaunchAgentSpec(
        label="com.openclaw.rocky-weekly-personal-review",
        plist_path="/Users/clawdbot/Library/LaunchAgents/com.openclaw.rocky-weekly-personal-review.plist",
        program_arguments=WEEKLY_PERSONAL_REVIEW_SSH_BRIDGE_PROGRAM_ARGUMENTS,
        working_directory="/Users/clawdbot/.openclaw/workspace",
        stdout_path="/Users/clawdbot/.openclaw/logs/rocky-weekly-personal-review.log",
        stderr_path="/Users/clawdbot/.openclaw/logs/rocky-weekly-personal-review.err.log",
        weekdays=[1],
        hour=7,
        minute=15,
        timezone="Europe/Prague",
    ...
    state_path="/Users/clawdbot/.openclaw/state/weekly_personal_review_scheduler.json",
    first_expected_run_after="2026-05-25T07:15:00+02:00",
)
...
MEETING_OUTCOME_CAPTURE_SPEC = SchedulerJobSpec(
    job_name="meeting_outcome_capture",
    job_label="Rocky meeting outcome capture",
    workflow="meeting_outcome_scheduler",
    launchagent=LaunchAgentSpec(
        label="com.openclaw.rocky-meeting-outcome-capture",
        plist_path="/Users/clawdbot/Library/LaunchAgents/com.openclaw.rocky-meeting-outcome-capture.plist",
        program_arguments=MEETING_OUTCOME_CAPTURE_SSH_BRIDGE_PROGRAM_ARGUMENTS,
        working_directory="/Users/clawdbot/.openclaw/workspace",
        stdout_path="/Users/clawdbot/.openclaw/logs/rocky-meeting-outcome-capture.log",
        stderr_path="/Users/clawdbot/.openclaw/logs/rocky-meeting-outcome-capture.err.log",
        weekdays=[1, 2, 3, 4, 5],
        hour=8,
        minute=0,
        timezone="Europe/Prague",
        first_expected_run_after="2026-05-25T08:00:00+02:00",
        start_interval_seconds=1800,
    ),
    state_path="/Users/clawdbot/.openclaw/state/meeting_outcome_scheduler.json",
    first_expected_run_after="2026-05-25T08:00:00+02:00",
...
def launchagent_execution_mode(program_arguments: list[str]) -> str:
    ...
    if (
        program_arguments[:1] == ["/usr/bin/ssh"]
        and "localhost" in program_arguments
        and "assistant_learning_scheduler.py --live" in joined
        and "--notify-failures" in joined
        and "--json" in joined
    ):
        return "localhost_ssh_bridge"
    if (
        program_arguments
        and program_arguments[0].endswith("/python")
        and any(arg.endswith("assistant_learning_scheduler.py") for arg in program_arguments)
        and "--live" in program_arguments
        and "--notify-failures" in program_arguments
        and "--json" in program_arguments
    ):
    ...
    if (
        program_arguments[:1] == ["/usr/bin/ssh"]
        and "localhost" in program_arguments
        and "weekly_personal_review_scheduler.py --live" in joined
        and "--notify" in joined
        and "--json" in joined
    ):
        return "localhost_ssh_bridge"
    ...
    if (
        program_arguments[:1] == ["/usr/bin/ssh"]
        and "localhost" in program_arguments
        and "meeting_outcome_scheduler.py --live" in joined
        and "--notify-failures" in joined
        and "--apply-safe-followups" in joined
        and "--json" in joined
    ):
        return "localhost_ssh_bridge"
    if (
    ...
    if (
        program_arguments
        and program_arguments[0].endswith("/python")
        and any(arg.endswith("meeting_outcome_scheduler.py") for arg in program_arguments)
        and "--live" in program_arguments
        and "--notify-failures" in program_arguments
...
def _assistant_learning_natural_run_status(
    spec: SchedulerJobSpec,
    *,
    now: datetime,
    launchagent: dict[str, Any],
    ...
    if now < grace_at:
        return {
            ...
            "expected_run": first_expected.isoformat(),
            "grace_until": grace_at.isoformat(),
            "summary": "First weekday learning run has not reached its grace window yet.",
        }
    verified = (
        target_date == expected_date
        and bool(last_run_at and last_run_at >= first_expected)
        and last_status in {"ok", "calibration_pending", "degraded_no_active_preferences", "skipped_insufficient_evidence"}
        and int(logs.get("stderr_size") or 0) == 0
        and last_exit_code in {0, "0"}
    )
    if verified:
        return {
            ...
            "last_run_at": last_run_at.isoformat() if last_run_at else None,
            "last_status": last_status,
            "summary": "First weekday assistant learning run is verified.",
        }
    return {
        "status": "natural_run_failed",
        "expected_run": first_expected.isoformat(),
        ...
        "last_status": last_status,
        "last_exit_code": last_exit_code,
        "summary": "First weekday assistant learning run is past grace and has not produced a clean verified state.",
    }
...
def evaluate_scheduler_job(
    ...
    *,
    ...
    if spec.job_name == "assistant_learning":
        natural_run = _assistant_learning_natural_run_status(
            spec,
            now=now,
            launchagent=signals.get("launchagent") or {},
            logs=logs,
        ...
        if natural_run.get("status") == "natural_run_failed":
            issues.append(
                {
                    "status": "degraded",
                    "failure_class": "assistant_learning_natural_run_failed",
                    "summary": natural_run.get("summary") or "Assistant learning first weekday natural run has not been verified.",
                }
            )
